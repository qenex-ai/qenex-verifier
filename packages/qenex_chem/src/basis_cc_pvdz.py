"""
cc-pVDZ Basis Set Parameters for Z=1-10
=========================================
Dunning correlation-consistent polarized valence double-zeta basis set.
...
Row-2 s-shells: 8 primitives → 2 general contractions + 1 uncontracted
Row-2 p-shells: 3 primitives → 1 contraction + 1 uncontracted
"""

import numpy as np

__all__ = ["CC_PVDZ", "RHF_CC_PVDZ_REFERENCE", "validate_basis", "get_basis_info"]
...
# PySCF 2.12.1 reference energies (cart=True) — will be re-validated after basis fix
RHF_CC_PVDZ_REFERENCE = {
    "He": -2.8551604772,
    "H2_1.4": -1.1287094490,
    "H2O": -76.0271401825,
    "Ne": -128.4888661720,
    "LiH_3.015": -7.9836534298,
}
...
def get_basis_info(element):
    if element not in CC_PVDZ:
        raise ValueError(
            f"Element '{element}' not in cc-pVDZ. Available: {sorted(CC_PVDZ.keys())}"
        )
...
def validate_basis(element):
    if element not in CC_PVDZ:
        return {"element": element, "checks": [f"FAIL: not in cc-pVDZ"]}
...
def validate_all():
    return all("ALL CHECKS PASSED" in validate_basis(e)["checks"] for e in CC_PVDZ)
